...
@testset "AJD.jl" begin
    ...
    @testset "Test for input assertions" begin
        Cs1 = ones(2, 2, 1)
        Cs1[:,:,1] = [1.0 0.2; 0.2 0.8]
        @test_throws DimensionMismatch ffdiag(Cs1[:,:,1])
        ...
        @test_throws DimensionMismatch ffdiag(Cs3)
    end
    ...
    @testset "Test Plot not nothing" begin
        errs = [0.1, 0.01, 0.001, 0.0001, 0.00001, 0.000001]
        plot = plot_convergence(errs, "ffdiag")
        @test !isnothing(plot)
    end
end
